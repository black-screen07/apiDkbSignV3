    ...
    try:
        ...
        for document in documents:
            ...
            if document.user.account_type == "individual":
                ...
                if not company:
                    continue
                subfolder = f"companies/{company.name.replace(' ', '_')}/users/{document.user.email}"
            else:
                continue
    ...
    try:
        ...
        if document.user_id:
            ...
            if user.account_type == "employee" and user.company_id:
                ...
                subfolder = f"companies/{company.name.replace(' ', '_')}/users/{user.email}"
            else:
        ...
        for signer in all_signers:
            ...
            elif signer.account_type == "external":
                contact_info = Contact.query.get(signer.signer_id)
                if contact_info:
...
def get_document_url_multiple():
    try:
        ...
        batch_uuid = request.args.get('uuid')
        if not batch_uuid:
            return jsonify({"error": "Batch UUID manquant"}), 400

        current_app.logger.info(f"Batch UUID reçu : {batch_uuid}")  # Log pour le débogage

        # Trouver tous les signataires avec ce batch_uuid
        signers = (
            Signer.query
            .join(Document, Document.id == Signer.document_id)
            .filter(Signer.batch_uuid == batch_uuid)
            .order_by(Document.created_at.desc())
            .all()
        )

        if not signers:
            return jsonify({"error": "Aucun signataire trouvé pour ce batch UUID"}), 404

        # Vérifier si au moins un signataire est en attente
        has_pending = False
        for signer in signers:
        ...
        if not has_pending:
            return jsonify({"error": "Aucun document en attente pour ce batch UUID"}), 403

        # Récupérer les documents associés
        document_ids = list(set(signer.document_id for signer in signers))
        documents = Document.query.filter(Document.id.in_(document_ids)).all()

        if not documents:
            return jsonify({"error": "Aucun document trouvé pour ce batch UUID"}), 404
        ...
        for document in documents:
            ...
            if document.user_id:
                ...
                current_app.logger.warning(f"Document {document.id} sans propriétaire")
                continue  # Ignorer les documents sans propriétaire
        ...
        # Retourner la réponse avec la liste des documents
        return jsonify({
            "message": f"{len(documents_data)} document(s) trouvé(s) avec succès",
